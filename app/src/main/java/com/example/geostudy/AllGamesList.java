...
//    }
    ...
    public String getLocalRegionName(RegionData data, String mapGame, String region) {
        String currentLang = getCurrentLanguage();
        try {
            if (data.getRegionsData() != null
                    && data.getRegionsData().containsKey(mapGame)
                    && data.getRegionsData().get(mapGame).containsKey(region)) {
                Map<String, String> translations = data.getRegionsData().get(mapGame).get(region);
                if (translations.containsKey(currentLang)) {
                    return translations.get(currentLang);
                } else {
...
//        russia.put("RU-KL", "Республика Калмыкия");
